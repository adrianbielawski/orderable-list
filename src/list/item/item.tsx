...
const Item: FC<Props<any>> = (props) => {
  ...
  const handleRemove = () => {
    setItemRemoved(true);
    timeout.current = setTimeout(() => setHeight(0), 400);
    timeout.current = setTimeout(() => props.onRemove!(props.index), 500);
  };
  ...
  const getDynamicStyles = () => {
    ...
    let styles: DynamicStyles = {
      transform:
        props.transform && `translate(0px, ${grabbedElement.height}px)`,
    };
  ...
  const itemClass = cx("item", props.className, {
    grabbed: isGrabbed,
  ...
  return (
    <ItemContext.Provider value={itemContext}>
      <li className={itemClass} style={getDynamicStyles()} ref={element}>
        <AnimateHeight className={contentClass} height={height} duration={100}>
          {props.children}
        </AnimateHeight>
      </li>
